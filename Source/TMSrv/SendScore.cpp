...
void FixSendScore_SERVER(BYTE *m_PacketBuffer, BYTE *packetBuffer)
{
	...
	nInfo* mob = &nBuffer[new_SendScore->Header.ClientId];
	...
	new_SendScore->Resist1 = player->Resists[0];
	new_SendScore->Resist2 = player->Resists[1];
	new_SendScore->Resist3 = player->Resists[2];
	new_SendScore->Resist4 = player->Resists[3];
}