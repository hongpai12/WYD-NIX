...
void FixGetCreateMob_SERVER(BYTE *m_PacketBuffer, BYTE *packetBuffer)
{
	...
	for (int i = 0; i < 16; i++)
	{
		new_GetCreateMob->Affect[i].Index = (BYTE)mBuffer[new_GetCreateMob->ClientId].Affects[i].Index;
		new_GetCreateMob->Affect[i].Time  = (BYTE)mBuffer[new_GetCreateMob->ClientId].Affects[i].Time;
	...
	if (new_GetCreateMob->ClientId <= MAX_PLAYER)
		strncpy(&new_GetCreateMob->Tab[0], nBuffer[new_GetCreateMob->ClientId].Ingame.Tab, 26);
	else
	{
		if (new_GetCreateMob->Header.ClientId <= MAX_PLAYER)
		{
			if (!nBuffer[new_GetCreateMob->Header.ClientId].Chars[nBuffer[new_GetCreateMob->Header.ClientId].Ingame.LoggedMob].Quests.Armeiro1)
				strncpy(&new_GetCreateMob->Tab[0], "Quest Disponível !!!", 26);
		}
		else
		{
			for (int i = 0; i < 50; i++)
			{
				...
				if (List->Dex == thisMob->bStatus.Points[DEXTE])
					strncpy(&new_GetCreateMob->Tab[0], List->Tab, 26);
			}
		}
	}
