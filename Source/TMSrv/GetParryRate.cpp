...
INT32 GetParryRate(st_Mob *player, INT32 Parry)
{
	...
	result[0] = player->Status.Points[DEXTE];
...
INT32 __stdcall HKD_GetParryRate_FixEvasion(INT32 clientid, INT32 otherClientId, INT32 parry)
{
	...
	{
		INT32 mCALCPLAYER = (INT32)(player->Status.Points[DEXTE] / 1000);
		if (mCALCPLAYER <= 0)
			mCALCPLAYER = 1;
		else if (mCALCPLAYER > 15)
			mCALCPLAYER = 15;

		INT32 mCALCAFFECTED = (INT32)(otherPlayer->Status.Points[DEXTE] / 1000);
		if (mCALCAFFECTED <= 0)
			mCALCAFFECTED = 1;
...
{
	//by shepher
	stAffects *affect = mBuffer[clientId].Affects;
	for (int i = 0; i<16; i++)
	{
		if (affect[i].Index == 8)
		{
